...
| **Preview branches cannot deploy** | Two attempts on `diag/history-minw-900` both failed 0/5 on prerender, with `/` itself timing out on `#hero-title`, while main built 5/5 in four seconds. Chromium cold-start in a build cache that does not have it warm. The gate correctly refused to ship five shell routes — that part is working. **The consequence is larger than one diagnostic: every future "send me a preview URL to test on the device" is blocked**, and iPhone work depends on exactly that loop. Likely fixes: pin/cache Chromium explicitly, raise the prerender watchdog and per-route waits for cold containers, or skip prerender on preview targets and gate only on production. | Weekly product slice | 2026-09-20 |
