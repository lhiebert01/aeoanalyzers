...
| **The retired `PI GenAI LLC` spelling, 17 instances across five published posts** | Inconsistent rather than false, so it meets neither interrupt exception, and touching published body text triggers G7. **Urgency note for whoever picks this up:** the new primer cross-links to all five of those posts and uses the canonical `PIGENAI LLC`, so a prospect who follows a link from the primer sees two spellings of the company name one click apart. When it is cleared it is ONE pass across all five files plus the syndicated copies, never a drip — a half-cleared set is worse than an uncleared one, because it looks like two companies rather than one typo. | Weekly product slice · founder word on published body text | 2026-09-20 |
